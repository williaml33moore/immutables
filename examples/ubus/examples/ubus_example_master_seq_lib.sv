...
class read_modify_write_seq extends ubus_base_sequence;
  ...
  virtual task body();
    ...
    addr_check = read_byte_seq0.rsp.get_request().addr;
    m_data0_check = read_byte_seq0.rsp.get_response().data().get(0) + 1;
    ...
    assert(m_data0_check == read_byte_seq0.rsp.get_response().data().get(0)) else
      `uvm_error(get_type_name(),
        $sformatf("%s Read Modify Write Read error!\n\tADDR: %h, EXP: %h, ACT: %h", 
        get_sequence_path(),addr_check,m_data0_check,read_byte_seq0.rsp.get_response().data().get(0)));
  endtask : body
